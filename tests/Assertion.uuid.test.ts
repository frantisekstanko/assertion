...
describe('Assertion.uuid', () => {
  ...
  it('should throw for invalid UUIDs', () => {
    expect(() => Assertion.uuid('not-a-uuid')).toThrow('Expected a valid UUID')
    expect(() => Assertion.uuid('123e4567-e89b-12d3-a456')).toThrow(
      'Expected a valid UUID',
    ) // too short
    expect(() =>
      Assertion.uuid('123e4567-e89b-12d3-a456-426614174000-extra'),
    ).toThrow('Expected a valid UUID') // too long
    expect(() => Assertion.uuid('123e4567e89b12d3a456426614174000')).toThrow(
      'Expected a valid UUID',
    ) // no dashes
    expect(() =>
      Assertion.uuid('123e4567-e89b-12d3-a456-42661417400g'),
    ).toThrow('Expected a valid UUID') // invalid character
    expect(() => Assertion.uuid('')).toThrow('Expected a valid UUID')
  })

  it('should throw for non-strings', () => {
    expect(() => Assertion.uuid(123)).toThrow(
      'Expected a UUID string, got number',
    )
    expect(() => Assertion.uuid(null)).toThrow(
      'Expected a UUID string, got null',
    )
    expect(() => Assertion.uuid(undefined)).toThrow(
      'Expected a UUID string, got undefined',
    )
    expect(() => Assertion.uuid({})).toThrow(
      'Expected a UUID string, got object',
    )
  })

  it('should throw custom message', () => {
    expect(() => Assertion.uuid('invalid', 'Custom error')).toThrow(
      'Custom error',
    )
  })
})
